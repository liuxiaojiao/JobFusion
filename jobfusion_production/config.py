class configure:
    ...
    resume_info_extract_role = 'Resume information extracter'
    resume_info_extract_agent_goal = 'Extract and structure resume information into JSON format'
    resume_info_extract_agent_bkground = """Expert at parsing documents and extracting structured information"""
    ...
    resume_Info_extract_task = f'''Extract information from the provided resume files including summary, work experience, education, skills, address,\
...
When you are done with the work, go back to the md file again and check if you have missed anything. If you have missed anything, go back to the resume files and extract the missing information.\
    '''
    resume_info_task_expected_output="Json file with all the extracted information from the resume files"
    resume_info_task_output_file='output/updated_resumev2.json'
    

    create_job_analyzer_role = 'Job Description Analyzer'
    create_job_analyzer_goal = 'Extract key requirements and qualifications from job postings'
    create_job_analyzer_bkground = """Specialist in analyzing job descriptions and identifying key requirements"""
    create_job_analyzer_task = f"Analyze job posting from URL: {job_url}"
    create_job_analyzer_output="Json file with all the extracted information from the resume files"
    create_job_analyzer_file='output/updated_resumev2.json'