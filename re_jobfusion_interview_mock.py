...
# Website Scraping Agent Function
def WEBScrapeWebsiteToolAgent(agent_role, agent_goal, agent_bk, task_goal, website_url):
    WEBSagent = Agent(
        role=agent_role,
        goal=agent_goal,
        backstory=agent_bk,
        llm=llm_35_turbo,
        tools=[ScrapeWebsiteTool(website_url=website_url)],
        allow_delegation=False,
        verbose=True,
        max_iter=5
    )
    
    task = Task(expected_output="{input}", description="complete the task in {input}", agent=WEBSagent)
    my_crew = Crew(agents=[WEBSagent], tasks=[task])
...
def main():
    ...
    if st.button('Start Processing'):
        if uploaded_resume and uploaded_personal_writeup:
    ...
    if uploaded_resume and uploaded_personal_writeup and jd_url_input:
        ...
        if db:
            ...
            qa_chain = get_qa_chain(
                ...
                user_personal_writeup=user_personal_writeup, job_qualifications=jd_result,
